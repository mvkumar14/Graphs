...
I would need to call it (with my current implimentation) < 100*10 = 1000 times. Because I am calling add_friendships only when the id of the current is less than the id of the friend. This means that the last person might not have add_friendships called at all. However, in my current implimentation it still checks to see if friendships need to be added on the last run.
